   ...
   return (
      <div id='o-nas' className='w-full'>
         <div className='relative w-full max-w-[1200px] mx-auto py-20 p-8 grid grid-cols-2 gap-2 md:gap-4'>
            {images.map((el, index) => (
               <img 
                  className='w-full max-md:h-[250px] object-cover object-center'
                  key={index}
                  src={el}
               />
            ))}
            <div className='absolute w-full h-full top-0 left-0 flex justify-center items-center p-14'>
               <div className='bg-white/90 w-[500px] flex flex-col gap-8 p-8 '>
                  <h1 className='text-xl uppercase font-extrabold md:text-center'>
                     O nas
                  </h1>
                  <p className='text-xs md:text-base font-semibold tracking-wider indent-10'>
